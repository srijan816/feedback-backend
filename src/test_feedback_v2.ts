/**
 * Test Feedback V2 Generation
 * Generates timestamped feedback with playable moments
 */
...
async function main() {
  ...
  console.log('TESTING FEEDBACK V2 - WITH PLAYABLE MOMENTS');
  console.log('='.repeat(80));
  console.log();

  // Read test input metadata
  const inputPath = path.join(__dirname, '../test_data/feedback_v2_input.json');

  if (!fs.existsSync(inputPath)) {
    console.error('❌ Test input not found. Run: python3 test_feedback_v2.py');
    process.exit(1);
  }
  ...
  console.log(`  Position: ${input.position || 'Unknown'}`);
  console.log(`  Duration: ${input.actual_time_seconds}s`);
  console.log();

  console.log('🔄 Generating V2 feedback...');
  console.log();
  ...
  try {
    const result: FeedbackV2Result = await generateFeedbackV2({
      ...
      motion: input.motion,
      position: input.position || 'Prop 1',
    ...
    result.playable_moments.forEach((moment, idx) => {
      ...
      console.log(`"${moment.what_they_said}"`);
      console.log();
      console.log(`Issue:`);
      console.log(moment.issue);
      console.log();
      console.log(`Recommendation:`);
      console.log(moment.recommendation);
      console.log();
    ...
    // Save result
    const outputPath = path.join(__dirname, '../test_data/feedback_v2_result.json');
